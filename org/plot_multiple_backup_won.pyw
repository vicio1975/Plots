...
def plot_chart(selected_column):
    global ax, canvas, current_chart_index

    ax.clear()
    ax.plot(filtered_df[0], filtered_df[selected_column], label=selected_column)
    ...
    current_chart_index = selected_column

def save_plot():
    if canvas:
        file_path = file_path_var.get()
        variable_name = columns_combobox.get()
...
def show_next_chart():
    ...
    selected_index = columns_combobox.current()
    if selected_index < len(variable_names) - 1:
        selected_index += 1
        columns_combobox.current(selected_index)
        clean_plot()
        plot_chart(columns_combobox.get())
...
if __name__ == "__main__":
    ...
    root.geometry("1000x510+200+150")
    ...
    columns_label = tk.Label(data_config_frame, text="Select Data Column:", font=f_H10)
    columns_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
    columns_combobox = ttk.Combobox(data_config_frame, state="readonly")
    columns_combobox.grid(row=2, column=1, padx=10, pady=5, sticky="w")
    update_available_columns()

    frame_fig = tk.LabelFrame(top_frame_1, text="", width=495, height=458, font=f_H10)
    frame_fig.grid(row=0, column=1, padx=10, pady=5, rowspan=4, sticky="w")
    ...
    next_button = tk.Button(button_frame, text="Next", command=show_next_chart, width=8)
    next_button.grid(row=0, column=1, padx=5, pady=8, sticky="w")

    prev_button = tk.Button(button_frame, text="Previous", command=show_previous_chart, width=8)
    prev_button.grid(row=0, column=2, padx=5, pady=8, sticky="w")

    clean_button = tk.Button(button_frame, text="Clean Plot", command=clean_plot, width=8)
