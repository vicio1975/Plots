...
def load_data():
    ...
    if file_path:
        ...
        try:
            ...
            print(f"Error: Unable to load data from {file_path} with delimiter='{delimiter_char}' "
                  f"and header_lines={selected_header_lines}")
...
def refresh_data():
    ...
    # If no file has been loaded yet, we can't refresh
    if not last_file_path:
        messagebox.showwarning("Warning", "No file has been loaded yet to refresh.")
        return

    # 1) Store current selection states (X or Y) and variable names
    selection_states = {}
    ...
    # Map the Tkinter combobox selection back to the actual delimiter character
    ...
    try:
        ...
        if datasets:
            datasets[-1] = loaded_data  # Replace last loaded dataset

        # 3) Re-populate checkboxes
        ...
        print(f"Error: Unable to parse data from {last_file_path}")
