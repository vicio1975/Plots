...
def populate_column_checkboxes():
    ...
    if loaded_data is not None:
        ...
        for idx in range(num_columns):
            ...
            column_name = tk.Label(checkboxes_frame, text=f"Column {idx}")
            column_name.grid(row=idx, column=2, sticky='w', padx=30, pady=2)
...
def plot_data():
    ...
    for idx, (checkbox_x, var_x, checkbox_y, var_y, var_name) in enumerate(column_checkboxes):
        if var_x.get():
...
if __name__ == "__main__":
    ...
    exit_button.grid(row=0, column=2, padx=15, pady=8)
